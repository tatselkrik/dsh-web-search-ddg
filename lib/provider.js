...
export function parseLiteResults(html) {
    const rows = [];
    const seen = new Set();
    const token = /<a\s+([^>]*\bhref\s*=\s*(["'])(.*?)\2[^>]*)>([\s\S]*?)<\/a>|<t[dh][^>]*class="?result-snippet"?[^>]*>([\s\S]*?)<\/t[dh]>/gi;
    let match;
    let pending;
    while ((match = token.exec(html)) !== null) {
        if (match[3] !== undefined) {
            if (!isResultAnchor(match[1] ?? ''))
                continue;
...
function isResultAnchor(attributes) {
...
export function buildRequestUrl(options, query) {
    ...
    if ((options.method ?? DDG_DEFAULT_METHOD) === 'get')
...
export class DdgSearchProvider {
    ...
    async search(request, signal) {
        ...
        const composite = AbortSignal.any(signal === undefined
